...
<body class="antialiased">
    ...
    <section class="relative w-full h-[80vh] mt-32 overflow-hidden">
        ...
        {{-- Tombol navigasi --}}
        <button id="prevBtn"
            class="absolute left-5 top-1/2 -translate-y-1/2 bg-black/40 text-white rounded-full p-3 hover:bg-black/60 z-20">
            &#10094;
        </button>
        <button id="nextBtn"
            class="absolute right-5 top-1/2 -translate-y-1/2 bg-black/40 text-white rounded-full p-3 hover:bg-black/60 z-20">
            &#10095;
        </button>
    </section>
    ...
    {{-- Hero Section --}}
    <section
        class="relative bg-cover bg-center bg-no-repeat h-[60vh] max-w-6xl mx-auto flex items-center overflow-hidden mt-20 mb-20"
    ...
    <section class="max-w-5xl mx-auto px-6 py-2 text-center">
        <h2
            class="font-poppins text-[80px] font-semibold normal-case not-italic leading-[70px] tracking-[0px] text-black no-underline">
            Tuition Fee</h2>
        <p class="text-black-600 text-[18px] max-w-2xl mx-auto mb-12 py-3">
            Welcome to the tuition fee guide for this academic year. <br> Here, you’ll find everything you need to
            know—like how much <br>
        ...
        <div id="admission">
            <section class="flex flex-col items-center text-center px-4 py-8 bg-white">
                ...
                <script>
                    fetch("http://127.0.0.1:8000/api/admissions")
                        .then(res => res.json())
                        .then(admissions => {
                            const container = document.getElementById('admission');
                            console.log(admissions);
                            admissions.data.forEach(p => {
                                container.innerHTML += `

        <div class="w-full max-w-4xl">
            <img src="http://127.0.0.1:8000/images/admissions/${p.image}" alt="${p.name}" class="w-full h-auto rounded-lg shadow-md" />
        </div>
      `;
                            });
                        })
        ...
        <section class="flex flex-col items-center justify-center text-center py-12 bg-white relative">
            ...
            <div class="mt-8 relative z-10 py-8">
                <a href="https://api.whatsapp.com/send/?phone=6281111816036&text=Hello%2C+I+want+to+know+more+about+IPTI&type=phone_number&app_absent=0"
                    target="_blank"
